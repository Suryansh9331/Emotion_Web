...
import { RSarticleContents } from "./data/articles";
...
//       const data = RSarticleContents[slug];
...
//       <Link to="/RSarticles" className="text-blue-500 hover:underline">&larr; Back to all articles</Link>
...
const RSArticleNews = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (!cleanSlug || !RSarticleContents[cleanSlug]) {
        setPFArticle(null);
        return;
      }
      setPFArticle(RSarticleContents[cleanSlug]);
    };
    load();
  }, [slug]);
