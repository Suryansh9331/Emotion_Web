...
import { MBarticleContents } from "./data/articles";
...
//       const data = MBarticleContents[slug];
...
//       <Link to="/MBarticles" className="text-blue-500 hover:underline">&larr; Back to all articles</Link>
...
const MBArticleNews = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (!cleanSlug || !MBarticleContents[cleanSlug]) {
        setPFArticle(null);
        return;
      }
      setPFArticle(MBarticleContents[cleanSlug]);
    };
    load();
  }, [slug]);
