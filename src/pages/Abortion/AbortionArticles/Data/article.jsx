export const Abortionarticles = [
  {
    slug: "2190-how-painful-is-a-medication-abortion",
    category: "FAQs - Abortion",
    title: "How painful is a medication abortion?",
    subtitle: null,

    image:
      "/SHW/faq_question_marks-12cc02f534646e9c41b2b62dd380b297bab076af0f938789e53822a88e551b81.svg",
    hasQuestionPattern: false,
  },
  {
    slug: "2189-how-long-does-an-abortion-take",
    category: "FAQs - Abortion",
    title: "How long does an abortion take?",
    subtitle:
      "",

    image:
      "/SHW/faq_question_marks-12cc02f534646e9c41b2b62dd380b297bab076af0f938789e53822a88e551b81.svg",
    hasQuestionPattern: false,
  },
  {
    slug: "2191-can-i-be-sedated-if-i-m-getting-an-abortion-procedure",
    category: " FAQs - Abortion",
    title: "Can I be sedated if I’m getting an abortion procedure?",
    
    image:
      "/SHW/faq_question_marks-12cc02f534646e9c41b2b62dd380b297bab076af0f938789e53822a88e551b81.svg",
    hasQuestionPattern: false,
  },
  {
    slug: "2177-what-does-viability-mean",
    category: "FAQs - Abortion",
    title: "What does “viability” mean?",
    subtitle: "",
    image:
      "/SHW/faq_question_marks-12cc02f534646e9c41b2b62dd380b297bab076af0f938789e53822a88e551b81.svg",
    hasQuestionPattern: false,
  },
 
  {
    slug: "2546-why-thanks-birth-control-day-matters-more-than-ever-this-year",
    category: " Birth Control",
    title: "Why Thanks, Birth Control Day matters more than ever this year",
    subtitle: "And how to make it the best ever",
    image:
      "/bithcontrolarticle/imgi_33_GettyImages-523793550 (1).jpg",
    hasQuestionPattern: false,
  },
  {
    slug: "2486-six-things-you-can-do-to-help-on-the-one-year-anniversary-of-roe-v-wade-being-overturned",
    category: "Abortion",
    title: "Six things you can do to help on the one-year anniversary of Roe v. Wade being overturned",
    subtitle: "Don’t give up, we still need your help!",
    image:
      "/Abortion/imgi_6_GettyImages-1241532935.jpg",
    hasQuestionPattern: false,
  },
 
  {
    slug: "2394-mourning-while-celebrating-the-50th-anniversary-of-roe-v-wade",
    category: " Abortion",
    title: "Mourning while celebrating the 50th anniversary of Roe v. Wade",
    subtitle: "A bittersweet reminder of what we had and all we’ve lost",
    image:
      "/Abortion/imgi_7_GettyImages-1047330148.jpg",
    hasQuestionPattern: false,
  },
   {
    slug: "2606-can-i-start-opill-right-after-having-an-abortion",
    category: " FAQs - Abortion",
    title: "Can I start Opill right after having an abortion?",
    subtitle: "I'm having irregular bleeding with a progestin-only birth control method.",
    image:
      "/Abortion/imgi_8_GettyImages-879057644.jpg",
    hasQuestionPattern: false,
  },
];



// Full article contents, keyed by slug
export const AbortionarticleContents = {
 "2190-how-painful-is-a-medication-abortion": {
  title: "How painful is a medication abortion?",
  subtitle: "",
  author: "Dec 01, 2021",
  date: " Updated: May 16, 2025",
  updated: "Medically reviewed Aug 2020 by Robin Watkins, CNM, WHNP-BC",
  image: "",
  showSocialIcons: true,
  content: `There is no one-size-fits-all answer for how painful a medication abortion is. Some people experience very little pain, like mild period cramps, and some people experience very intense pain, which feels more like the pain of the contractions you’d have when giving birth.
<br>
If you’re having a medication abortion, you can take over-the-counter pain medication, like ibuprofen, or you can ask your provider to prescribe pain medication for you to take at home. In addition to pain medicine, you may find that using a heating pad, taking a hot shower, or other techniques like breathing exercises and meditation can also help with pain.
<br>
While the pain with a medication abortion can last longer than with an abortion procedure, many people find that the pain improves after they pass the pregnancy. If your pain isn’t getting better or is getting worse after you pass the pregnancy, contact the clinic where you had your abortion.`
}
,

  "2189-how-long-does-an-abortion-take": {
    title: "How long does an abortion take?",
    subtitle: "",
    author: "Medically reviewed Aug 2020 by Robin Watkins, CNM, WHNP-BC",
    date: "Dec 01, 2021",
    updated: "Updated: Jan 23, 2025",
    image:
      "",
    showSocialIcons: true,
    content: `How long an abortion takes depends on what type of abortion it is. With the abortion pill, you take two different medications. The first medication stops the pregnancy from continuing to develop, and the second medication starts the abortion process. You take the second one someplace safe and comfortable with easy access to a bathroom up to three days after taking the first medication. Once you take the second medication, the abortion usually starts within an hour and can take up to several hours to be complete. With the abortion procedure, the actual abortion typically only takes five to ten minutes.`
  }
,
  "2191-can-i-be-sedated-if-i-m-getting-an-abortion-procedure": {
    title: "Can I be sedated if I’m getting an abortion procedure?",
    subtitle: "",
    author: "Medically reviewed Aug 2020 by Robin Watkins, CNM, WHNP-BC",
    date: "Dec 01, 2021",
    updated: " Updated: Sep 16, 2024",
    image: "",
    showSocialIcons: true,
    content: `If you’re having an abortion procedure, you may have the option to have sedation during your procedure. One way to do this is for your abortion provider to give you anti-anxiety medication that you take by mouth before the procedure to relax you. You will be awake but should be more relaxed than usual. If you have what’s called “conscious sedation”, you’ll be awake but extremely relaxed, and the procedure should be virtually pain-free. If you have general anesthesia, which is a type of IV sedation that puts you to sleep completely, you won’t feel or experience anything during the procedure—you’ll simply wake up and it will be over. Sedation generally costs extra.

`,
  },

  "2177-what-does-viability-mean": {
    title: "What does “viability” mean?",
    subtitle: "",
    author: "Medically reviewed Aug 2020 by Robin Watkins, CNM, WHNP-BC",
    date: "Nov 30, 2021",
    updated: "Updated: Sep 16, 2024",
    image: "",
    showSocialIcons: true,
    content: `If you’re living in a state with an abortion ban, you’re probably wondering what viability means. Fetal viability is the point at which the fetus is likely to be able to live outside the uterus (with medical support). It usually occurs between 24 and 28 weeks of pregnancy.`,
  },

  "2606-can-i-start-opill-right-after-having-an-abortion": {
    title: "Can I start Opill right after having an abortion?",
    subtitle: "",
    author: "",
    date: "Mar 05, 2024",
    updated: "",
    image: "",
    showSocialIcons: true,
    content: `It depends on what type of abortion you have. If you have an abortion procedure, it’s totally safe (and often convenient!) to start birth control on the same day as your abortion. That means you can start using Opill (a new progestin-only birth control pill that is available over the counter) or another brand of pill that same day.
<br>
If you have a medication abortion (sometimes called “the abortion pill”), you’ll be able to start Opill or another hormonal method of birth control immediately after your abortion is complete (that is, as soon as you’ve finished the heavier bleeding that starts after the last pills).

`,
  },

  "2546-why-thanks-birth-control-day-matters-more-than-ever-this-year": {
    title: "Why Thanks, Birth Control Day matters more than ever this year",
    subtitle: "And how to make it the best ever",
    author: "",
    date: "Nov 10, 2023",
    updated: "",
    image:
      "/bithcontrolarticle/imgi_33_GettyImages-523793550 (1).jpg",
    showSocialIcons: true,
    content: `
<b>Why This Year Matters More Than Ever</b><br><br>
As we prepare for Thanks, Birth Control Day on Wednesday, November&nbsp;15th, we reflect on how much has changed since the movement began 11&nbsp;years ago.<br><br>
Back then, Roe v. Wade still protected abortion rights. Since its fall in June&nbsp;2022, we've seen bans and restrictions escalate across many states. Justice Clarence Thomas even hinted at reviewing the right to contraception. This era has created fear and uncertainty, even among long-time advocates.<br><br>

<b>Rebuilding Hope</b><br><br>
- Voters in California, Michigan, Ohio, and Vermont approved constitutional abortion rights.<br>
- Voters in Kansas, Kentucky, and Montana rejected stricter abortion proposals.<br>
- Awareness of AbortionFinder.org soared.<br>
- Local abortion funds received widespread support.<br>
- The FDA approved the first over-the-counter birth-control pill, increasing accessibility.<br><br>

<b>Let’s Make Some Noise This Year</b><br><br>
Now more than ever, we must act. Share your story. Educate others. Demand change from elected leaders. Every action matters in the fight for reproductive freedom.<br><br>
Campaigns like <b>#ThxBirthControl</b> are essential. Our health, future, and freedom depend on it.<br><br>

<b>How You Can Support #ThxBirthControl and Reproductive Freedom</b><br><br>

<b>1. Share Your Story</b><br>
Speak up about why birth control matters to you. Use <b>#ThxBirthControl</b> on social media. Discuss reproductive rights with friends and family. Your story has the power to inspire and normalize these critical conversations.<br><br>

<b>2. Contact Your Representatives</b><br>
Let your lawmakers know you support:<br>
- Improved insurance coverage for contraception<br>
- Increased family-planning funding<br>
- Protection of abortion access<br><br>
Call or email them. Share your values and urge them to back reproductive-rights legislation.<br><br>

<b>3. Donate or Volunteer</b><br>
Support organizations like <b>Power to Decide</b>. Your donations help sustain platforms like Bedsiderr.org and AbortionFinder.org.<br><br>
Prefer giving your time? Volunteer at a local abortion clinic, possibly as an escort.<br><br>

<b>4. Vote in Every Election</b><br>
Reproductive freedom is on the ballot in every election—federal, state, and local.<br><br>
Make sure you're registered. Vote. Encourage others to vote. Local leaders have a big impact on access to care in your community.<br><br>

<b>Together, We Can Make a Difference</b><br><br>
Through collective action—whether by voting, donating, volunteering, or simply telling our stories—we can protect reproductive rights and expand access to birth control.<br><br>
<b>Our future depends on it.</b>
    `
  
},
   "2486-six-things-you-can-do-to-help-on-the-one-year-anniversary-of-roe-v-wade-being-overturned": {
    title: "Six things you can do to help on the one-year anniversary of Roe v. Wade being overturned",
    subtitle: "Don’t give up, we still need your help!",
    author: "",
    date: "Jun 23, 2023",
    updated: "",
    image:
      "/Abortion/imgi_6_GettyImages-1241532935.jpg",
    showSocialIcons: true,
    content: `
<b>One Year Since Roe v. Wade Was Overturned</b><br><br>
It’s hard to believe, but tomorrow marks one year since Roe v. Wade was overturned.<br><br>

<b>What Was Roe v. Wade?</b><br><br>
Roe v. Wade was the 1973 landmark case that established the constitutional right to abortion and vastly improved access to abortion across the country. But on June 24, 2022, the U.S. Supreme Court’s decision in Dobbs v. Jackson Women’s Health Organization overturned Roe, opening the door for state legislators to limit or eliminate abortion access in their states.<br><br>

<b>The Impact Over the Past Year</b><br><br>
It’s easy to feel discouraged and angry watching one state after another ban or severely limit abortion. The ripple effect is still ongoing. States continue to pass laws restricting access.<br><br>

<b>What We're Doing</b><br><br>
At AbortionFinder, our sister site, we’ve continued fighting to help people find abortion providers, get financial support, and stay informed about state laws. And we’re not stopping.<br><br>

<b>Here’s How You Can Help</b><br><br>

<b>1. Donate to AbortionFinder</b><br>
Donating to Power to Decide (our parent site) helps us keep AbortionFinder going and support increased access to abortion. Every amount counts.<br><br>

<b>2. Tell a Friend</b><br>
If you know someone who needs an abortion—especially in a restricted state—tell them about AbortionFinder.org. Share the link with three friends. You never know who might need it.<br><br>
<b>3. Buy AbortionFinder Merch</b><br>
Show your support and spread awareness by purchasing merch like our tote bag. Check out the store for more items to wear proudly.<br><br>

<img src="/Abortion/PTD246.png" alt="AbortionFinder Merch" class="w-full h-auto mb-4"><br><br>
<b>4. Post on Social Media</b><br>
We’ve made it easy. Just share one of our graphics and tag @AbortionFinder. Your post helps raise awareness and support the fight.<br><br>

<b>5. Enter Our Giveaway</b><br>
Visit our Instagram to enter our giveaway for a chance to win an “Abortion Positive” hat, tote, and pin set. Share with a friend too!<br><br>

<b>6. Spread the Word About Local Abortion Funds</b><br>
Visit your state’s page on AbortionFinder to locate local abortion funds. Share their work so they can continue helping those in need.<br><br>

<b>Keep Fighting</b><br><br>
There’s still a long road ahead, and we need your help. Don’t give up—we certainly won’t.
    `
  },

   "2394-mourning-while-celebrating-the-50th-anniversary-of-roe-v-wade": {
    title: "Mourning while celebrating the 50th anniversary of Roe v. Wade",
    subtitle: "A bittersweet reminder of what we had and all we’ve lost",
    author: "",
    date: "Jan 20, 2023",
    updated: "",
    image:
      "/Abortion/imgi_7_GettyImages-1047330148.jpg",
    showSocialIcons: true,
    content: `What should’ve been a moment of pure celebration—the 50th anniversary of Roe v. Wade, which is this Sunday, January 22—has become an occasion for mourning as well as bittersweet reflection on what this landmark case brought us over almost 50 years.
<br>
Roe v. Wade established the constitutional right to abortion and, in so doing, vastly improved access to abortion across the country. But on June 24, 2022, the U.S. Supreme Court’s decision in Dobbs v. Jackson Women’s Health Organization overturned Roe v. Wade.
<br>
We will certainly cry this weekend (and probably also scream), but today, in defiance of those who have celebrated taking away one of our most precious rights, we have chosen to lift up the voices of those who have benefited from access to abortion.`
  },
};

export const AbortionarticlesTwo = [
  {
    slug: "having-pcos-taught-me-how-to-advocate-for-myself",
    category: "Periods & vaginal health",
    title:
      "Having PCOS taught me how to advocate for myself",
          subtitle: "",

    image:
      "https://www.Bedsiderr.org/rails/active_storage/representations/redirect/eyJfcmFpbHMiOnsibWVzc2FnZSI6IkJBaHBBakk5IiwiZXhwIjpudWxsLCJwdXIiOiJibG9iX2lkIn19--8ff4ba326b9e06a31687c92f3edd599f95a168ea/eyJfcmFpbHMiOnsibWVzc2FnZSI6IkJBaDdCem9MWm05eWJXRjBPZ2wzWldKd09oTnlaWE5wZW1WZmRHOWZabWxzYkZzSGFRTDRBbWtDa0FFPSIsImV4cCI6bnVsbCwicHVyIjoidmFyaWF0aW9uIn19--a3ab3c05229ad5f0864920d0b6b36ed3f914792c/GettyImages-1027444054.jpeg",
    hasQuestionPattern: false,
  },
  {
    slug: "how-to-clean-your-vagina",
    category: "Periods & vaginal health",
    title: "How to clean your vagina",
    subtitle: "We asked an expert so you don't have to",

    image:
      "https://www.Bedsiderr.org/rails/active_storage/representations/redirect/eyJfcmFpbHMiOnsibWVzc2FnZSI6IkJBaHBBbjRVIiwiZXhwIjpudWxsLCJwdXIiOiJibG9iX2lkIn19--08fee9633737d0c4a6769946c9223885c39829a6/eyJfcmFpbHMiOnsibWVzc2FnZSI6IkJBaDdCem9MWm05eWJXRjBPZ2wzWldKd09oTnlaWE5wZW1WZmRHOWZabWxzYkZzSGFRTDRBbWtDa0FFPSIsImV4cCI6bnVsbCwicHVyIjoidmFyaWF0aW9uIn19--a3ab3c05229ad5f0864920d0b6b36ed3f914792c/GettyImages-1169607304.jpg",
    hasQuestionPattern: false,
  },
  {
    slug: "if-you-have-a-vagina-you-have-vaginal-discharge",
    category: " Periods & vaginal health",
    title: "If you have a vagina, you have vaginal discharge",
    subtitle: "What’s normal, what’s not, and when to see your health care provider",
    image:
      "https://www.Bedsiderr.org/assets/mobile/provider_perspectives_header-44b4f94e01ec610c3de2b52585ef601d43bb8196a7253ae8e24896c8838e380a.jpg",
    hasQuestionPattern: false,
  },
  {
    slug: "what-is-your-vagina-supposed-to-smell-like",
    category: " Periods & vaginal health",
    title: "What is your vagina supposed to smell like?",
    subtitle:
      "Hint: not flowers!",
    image:
      "https://www.Bedsiderr.org/rails/active_storage/representations/redirect/eyJfcmFpbHMiOnsibWVzc2FnZSI6IkJBaHBBdklVIiwiZXhwIjpudWxsLCJwdXIiOiJibG9iX2lkIn19--522139e2fb60e7448c7effc665b32f841c1d1d0a/eyJfcmFpbHMiOnsibWVzc2FnZSI6IkJBaDdCem9MWm05eWJXRjBPZ2wzWldKd09oTnlaWE5wZW1WZmRHOWZabWxzYkZzSGFRTDRBbWtDa0FFPSIsImV4cCI6bnVsbCwicHVyIjoidmFyaWF0aW9uIn19--a3ab3c05229ad5f0864920d0b6b36ed3f914792c/GettyImages-1074280088.jpg",
    hasQuestionPattern: false,
  },
  {
    slug: "what-to-do-when-sex-hurts",
    category: " Periods & vaginal health",
    title:
      "What to do when sex hurts",
    subtitle: "Dyspareunia, or pain during sex, is more common than you might think",
    image:
      "https://www.Bedsiderr.org/rails/active_storage/representations/redirect/eyJfcmFpbHMiOnsibWVzc2FnZSI6IkJBaHBBaGtWIiwiZXhwIjpudWxsLCJwdXIiOiJibG9iX2lkIn19--a4b33ec04272adf37a3340d4a907017fe99f06ac/eyJfcmFpbHMiOnsibWVzc2FnZSI6IkJBaDdCem9MWm05eWJXRjBPZ2wzWldKd09oTnlaWE5wZW1WZmRHOWZabWxzYkZzSGFRTDRBbWtDa0FFPSIsImV4cCI6bnVsbCwicHVyIjoidmFyaWF0aW9uIn19--a3ab3c05229ad5f0864920d0b6b36ed3f914792c/Closed_Body-pillow_146A3631.jpg",
    hasQuestionPattern: false,
  },
];



