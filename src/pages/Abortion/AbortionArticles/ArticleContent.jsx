...
import { AbortionarticleContents } from './Data/article';
...
//       const data = AbortionarticleContents[slug];
...
const ArticleContent = () => {
  ...
  const [PFarticle, setPFArticle] = useState(null);
   ...
   const matchingArticle = Abortionarticles.find((article) => article.slug === slug);
...
const handleFeedbackClick = (type) => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (!cleanSlug || ! AbortionarticleContents[cleanSlug]) {
        setPFArticle(null);
        return;
      }
      setPFArticle( AbortionarticleContents[cleanSlug]);
    };
    load();
  }, [slug]);
     ...
     <div className="max-w-4xl mx-auto bg-white">
       {/* Back Navigation */}
         <div className="px-6 pt-6 pb-2">
          ...
          >
             {matchingArticle?.category || "Unknown"} &gt;&gt;
          </Link>
        </div>
             {/* Article Title */}
